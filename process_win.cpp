...
namespace TinyProcessLib
{
	...
	{
		int size = MultiByteToWideChar(CP_UTF8, 0, strU8.c_str(), -1, nullptr, 0);
		if (!size)
			return L"";

		std::wstring strU16;
		strU16.resize(size);
		MultiByteToWideChar(CP_UTF8, 0, strU8.c_str(), -1, static_cast<wchar_t*>(&strU16[0]), size);
		strU16.pop_back();
	...
	Process::id_type Process::open(const string_type &command, const string_type &path) noexcept
	{
		...
		std::wstring process_command_u = utf8to16(process_command);
		std::wstring path_u = utf8to16(path);
		BOOL bSuccess = CreateProcessW(
			nullptr,
			process_command.empty() ? nullptr : &process_command_u[0],
			nullptr, nullptr, TRUE, 0, nullptr,
			path.empty() ? nullptr : path_u.c_str(),
